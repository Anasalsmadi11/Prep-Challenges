...
const objLat = (obj) => {
    ...
    let fLetter = obj.firstName.charAt(0).toUpperCase() + obj.firstName.slice(1)
    let fLast = obj.lastName.charAt(0).toUpperCase() + obj.lastName.slice(1)

    return (`my name is ${fLetter} ${fLast} I am ${obj.age} YO, and I love ${obj.hobby}.`)
};
// -------------------------------------------------------------------------------------------------------
...
const cvFormatter = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        let skill = arr[i].tech

        if (arr[i].yearsOfExperience > 1) {

            if (arr[i].firstName != null && arr[i].lastName != null) {
                combined = arr[i].firstName + " " + arr[i].lastName
            } else if (arr[i].firstName == null && arr[i].lastName != null) {
                combined = arr[i].lastName
            }
            else if (arr[i].firstName != null && arr[i].lastName == null) {
                combined = arr[i].firstName
            }
            // console.log(combined)
            let obj = { fullName: combined, tech: skill }
            // console.log(obj)
            newArr.push(obj)
        }
    }
